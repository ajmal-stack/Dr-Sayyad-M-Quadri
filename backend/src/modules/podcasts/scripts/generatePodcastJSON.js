...
/**
 * Generate podcast JSON file for testing without MongoDB
 * This creates a properly formatted JSON file that matches the frontend structure
 */
...
console.log('🎧 Generating podcast JSON file...');

// Create the output directory if it doesn't exist
const outputDir = path.join(process.cwd(), '../../../../client/src/data');
const outputFile = path.join(outputDir, 'podcasts_generated.json');
...
try {
  ...
  console.log('✅ Successfully generated podcast JSON file!');
  console.log('📄 File location:', outputFile);
  console.log('📊 Episodes generated:', podcastData.episodes.length);
  console.log('🏷️  Categories:', [...new Set(podcastData.episodes.map(e => e.category))].join(', '));
  console.log('⭐ Featured episodes:', podcastData.episodes.filter(e => e.featured).length);
  
  console.log('\n🎯 Next steps:');
  console.log('1. Set up MongoDB (local or Atlas)');
  console.log('2. Run the importPodcastData.js script');
  console.log('3. Test the APIs with Postman');
