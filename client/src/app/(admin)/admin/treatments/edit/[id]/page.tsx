...
export default function EditTreatmentPage() {
  ...
  const loadTreatment = async () => {
    try {
      setLoading(true);
      const response = await treatmentsApi.getById(treatmentId);

      if (response.success) {
        const treatment: Treatment = response.data;
        setFormData({
          name: treatment.name || '',
          description: treatment.description || '',
          detailedDescription: treatment.detailedDescription || '',
          category: treatment.category || '',
          subcategory: treatment.subcategory || '',
          image: treatment.image || '',
          gradient: treatment.gradient || 'from-blue-500 to-indigo-600',
          icon: treatment.icon || 'HeartIcon',
          duration: treatment.duration || '8-12 sessions',
          methods: treatment.methods || [],
          conditions: treatment.conditions || [],
          keyPoints: treatment.keyPoints || [],
          metaTitle: treatment.metaTitle || '',
          metaDescription: treatment.metaDescription || '',
          keywords: treatment.keywords || [],
          status: treatment.status || 'draft',
          featured: treatment.featured || false,
          active: treatment.active !== undefined ? treatment.active : true,
          sessionCost: treatment.pricing?.sessionCost?.toString() || '',
          packageCost: treatment.pricing?.packageCost?.toString() || '',
          insuranceAccepted: treatment.pricing?.insuranceAccepted !== undefined ? treatment.pricing.insuranceAccepted : true,
          inPerson: treatment.availability?.inPerson !== undefined ? treatment.availability.inPerson : true,
          telehealth: treatment.availability?.telehealth !== undefined ? treatment.availability.telehealth : true,
          emergency: treatment.availability?.emergency || false,
        });
        
        // Set image preview if exists
        if (treatment.imageCloudinary?.url) {
          setImagePreview(treatment.imageCloudinary.url);
        } else if (treatment.image) {
          setImagePreview(treatment.image);
        }
      } else {
        showSnackbar('Failed to load treatment', 'error');
      }
    } catch (error: any) {
      console.error('Error loading treatment:', error);
      showSnackbar(error.message || 'Failed to load treatment', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        response = await treatmentsApi.update(treatmentId, formDataToSend);
      } else {
        ...
        response = await treatmentsApi.update(treatmentId, treatmentData);
      }
      ...
      if (response.success) {
        ...
        showSnackbar(response.message || 'Failed to update treatment', 'error');
      }
    } catch (error: any) {
      console.error('Error updating treatment:', error);
      showSnackbar(error.message || 'Failed to update treatment', 'error');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 4 }}>
        <Breadcrumbs sx={{ mb: 2 }}>
          ...
          <Typography color="text.primary">Edit Treatment</Typography>
        </Breadcrumbs>
                    ...
                    InputProps={{
                      endAdornment: (
                        <InputAdornment position="end">
                          <IconButton onClick={() => handleAddItem('methods', methodInput, setMethodInput)}>
                            <AddIcon />
                          </IconButton>
                        </InputAdornment>
                      ),
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    InputProps={{
                      endAdornment: (
                        <InputAdornment position="end">
                          <IconButton onClick={() => handleAddItem('conditions', conditionInput, setConditionInput)}>
                            <AddIcon />
                          </IconButton>
                        </InputAdornment>
                      ),
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    InputProps={{
                      endAdornment: (
                        <InputAdornment position="end">
                          <IconButton onClick={() => handleAddItem('keyPoints', keyPointInput, setKeyPointInput)}>
                            <AddIcon />
                          </IconButton>
                        </InputAdornment>
                      ),
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    InputProps={{
                      endAdornment: (
                        <InputAdornment position="end">
                          <IconButton onClick={() => handleAddItem('keywords', keywordInput, setKeywordInput)}>
                            <AddIcon />
                          </IconButton>
                        </InputAdornment>
                      ),
