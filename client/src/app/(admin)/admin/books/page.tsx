  ...
  const loadInitialData = async () => {
    try {
      ...
      try {
        ...
        // Fallback to basic structure if API not available
        booksData = [];
        setBooks(booksData);
        setFilteredBooks(booksData);
      }

      // Load categories from the new API
      try {
  ...
  const handleSaveBook = async () => {
    ...
    try {
      ...
      const sanitizedBook: BookPayload = {
        ...
        type: selectedBook.type || 'Books',
        price: Number(selectedBook.price) || 0,
        format: selectedBook.format && selectedBook.format.length > 0 ? selectedBook.format : ['E-book'], // Default to E-book if no format specified
        ...
        image: selectedBook.coverImage || selectedBook.image,
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  label="Status"
                  onChange={(e) => setSelectedBook({...selectedBook, status: e.target.value as 'draft' | 'review' | 'published' | 'archived'})}
                  disabled={dialogMode === 'view'}
                >
                  {statuses.map((status) => (
                    <MenuItem key={status} value={status}>
